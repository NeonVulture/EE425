...
Loop
		...
		;
		MOVLF 4,counter ; Period Selector
	...
	label_A
		...
		; BEGIN WRTING CODE HERE 
			...
			addwf xn,W ; x[n]+x[n-2]
			movwf SUM1L ; Store above result into SUM1L
			movf TEMP1,W
			addwfc TEMP2,W ;x[n]+x[n-2] 
			movwf SUM1U ;result into SUM1U

			; Second Summation (SUM2)
			movf xn4,W ; x[n-4] = WREG
			addwf xn6,W ; x[n-4]+x[n-6]
			movwf SUM2L ;result into SUM2L
			movf TEMP1,W
			addwfc TEMP2,w ;x[n-4]+x[n-6] = SUM2U
