...
;
; Program to implement y[n] = 0.5x[n] - 0.5x[n-1]
;
...
Loop
	...
	label_A
		...
		movf TABLAT, W
		movwf temp
		movff temp, value ; Get Current Value
		...
		; BEGIN WRTING CODE HERE 
			...
			;movff TABLAT, preVal2 ; preVal2 = currVal
            ;movff TABLAT, curVal ; curVal = TABLAT
            movff TBLPTRL,ProgMem ;save pointer location
			;change pointer to 4 steps back, points to x[n-2]
			tblrd*-
			tblrd*-
			tblrd*- 
			tblrd*- 
            ...
            ;addwfc preVal2, curVal, W ; Compute x[n] + x[n-2]
            ;rrcf WREG, W ; Divide by 2 via rotation to get Final Value
			;movwf z
...
Initial
        ...
        ;MOVLF  B'01100100', curVal ; x[n] = 100
		;MOVLF  B'00110010', preVal ; x[n-1] = 0
        ;MOVLF  B'00000000', preVal2 ; x[n-2] = 0
